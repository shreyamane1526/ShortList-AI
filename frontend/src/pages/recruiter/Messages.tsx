...
function buildThreads(messages: Message[], myId: number): Thread[] {
  ...
  for (const m of messages) {
    ...
    if (!map.has(partnerId)) map.set(partnerId, { partnerId, partnerName, messages: [], unread: 0 })
  ...
  async function openThread(thread: Thread) {
    ...
    thread.messages.forEach(async m => {
      if (!m.is_read && m.recipient_id === user?.id) {
        try { await api.patch(`/messages/${m.id}/read`) } catch { /* ignore */ }
      }
    })
  }
          ...
          <div className="flex-1 flex flex-col bg-gray-50">
            ...
            <div className="bg-white border-t border-gray-200 p-4">
              <div className="flex gap-3">
                ...
                />
                <button onClick={sendMessage} disabled={sending || !body.trim()} className="btn-primary self-end px-4">
